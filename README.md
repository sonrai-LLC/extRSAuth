...
**:::::UPDATE:::::** - A .ps1 script has been added to the project (toggleAuth.ps1, which toggles RS Authentication between Windows Auth and Custom Forms (ExtRSAuth implementation) via: .\toggleAuth), which can be executed after installation of ExtRSAuth via InitalizeExtRSAuth.ps1. Note that you must Stop the Report Server before you run .\toggleAuth. Adjust the toggleAuth.ps1 script as needed if any paths or other settings do not apply to your RS installation.
